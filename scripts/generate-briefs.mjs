...
const factsJson = execSync(
	`cd "${projectRoot}" && pnpm exec tsx -e 'import { facts } from "./src/lib/server/facts.ts";import { planReel, reelRequestSchema } from "./src/lib/server/reelPlanner.ts";const data = facts.map((f, i) => {  const dur = f.riskLevel === "high" ? 60 : f.riskLevel === "medium" ? 45 : 30;  const req = reelRequestSchema.parse({ factId: f.id, tone: "mentor", targetDurationSec: Math.min(180, Math.max(30, dur)) });  const brief = planReel(f, req);  return { slug: "reel-" + String(i+1).padStart(2,"0") + "-" + f.id, factId: f.id, rank: f.rank, title: f.title, brief };});process.stdout.write(JSON.stringify(data));'`,
